...
const create_walker_profile = async (req, res) => {
  try {
    ...
    if (!/^\d{9}$/.test(phone)) {
      return res
        .status(400)
        .json({ error: true, msg: "El teléfono debe tener 9 dígitos" });
    }
    if (password.length < 8 || password.length > 15) {
      return res
        .status(400)
        .json({ error: true, msg: "Contraseña 8–15 caracteres" });
    }
    ...
    if (!req.file) {
      return res
        .status(400)
        .json({ error: true, msg: "Foto de perfil obligatoria" });
    }
    if (isNaN(experience) || experience < 1 || experience > 99) {
      return res
        .status(400)
        .json({ error: true, msg: "Experiencia debe ser entero 1–99" });
    }
    if (!ALLOWED_WALKER_TYPES.includes(walker_type)) {
      return res
        .status(400)
        .json({ error: true, msg: "Tipo inválido: esporádico o fijo" });
    }
    if (!ALLOWED_ZONES.includes(zone)) {
      return res
        .status(400)
        .json({ error: true, msg: "Zona inválida: norte, centro o sur" });
    }
    if (description && description.length > 250) {
      return res
        .status(400)
        .json({ error: true, msg: "Descripción 50–250 caracteres" });
    }
...
const get_profile_by_id = async (req, res) => {
  try {
    ...
    if (role_id !== 1 && !(role_id === 2 && profile.walker_id === authUserId)) {
...
const approve_change_request = async (req, res) => {
  ...
  const p = await walker_profile.findByPk(id, {
    include: [
      { model: user, as: "user", attributes: ["user_id", "email", "name"] },
    ],
  });
  if (!p || !p.update_requested) {
    return res
      .status(404)
      .json({ msg: "Solicitud no encontrada", error: true });
  }
...
const reject_change_request = async (req, res) => {
  ...
  const emailText =
    "Tus cambios han sido rechazados. Por favor, revísalos de nuevo.";
  sendNotification(p.walker_id, {
